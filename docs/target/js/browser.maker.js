 ...
 *   contributors: Ben Zuill-Smith <bzuillsmith@gmail.com>
...
(function (MakerJs) {
    ...
    (function (importer) {
        ...
        function fromDXF(dxfContent, options) {
            if (options === void 0) { options = {}; }
            var dxfParser = require('dxf-parser');
            var parser = new dxfParser();
            var model = { paths: {}, models: {} };
            try {
                var dxf = parser.parseSync(dxfContent);
                if (!(dxf === null || dxf === void 0 ? void 0 : dxf.entities))
                    return model;
                // Get units from DXF file or use provided units
                var sourceUnits = determineSourceUnits(dxf, options.units || 'mm');
                var targetUnits = options.targetUnits || sourceUnits;
                // Calculate conversion factor
                var conversionFactor_1 = getConversionFactor(sourceUnits, targetUnits);
                // Process by layer to maintain order
                var layerGroups_1 = new Map();
                dxf.entities.forEach(function (entity) {
                    var _a;
                    var layer = entity.layer || 'default';
                    if (!layerGroups_1.has(layer)) {
                        layerGroups_1.set(layer, []);
                    }
                    (_a = layerGroups_1.get(layer)) === null || _a === void 0 ? void 0 : _a.push(entity);
                });
                layerGroups_1.forEach(function (entities, layer) {
                    var layerModel = { paths: {}, models: {} };
                    entities.forEach(function (entity) {
                        return addEntityToModel(layerModel, entity, layer, conversionFactor_1);
                    });
        ...
        importer.fromDXF = fromDXF;
        function determineSourceUnits(dxf, defaultUnits) {
            ...
            if (dxf.header && dxf.header.$INSUNITS) {
                ...
                var dxfUnits = dxf.header.$INSUNITS;
        ...
        function getConversionFactor(sourceUnits, targetUnits) {
            return unitConversions[sourceUnits] / unitConversions[targetUnits];
        }
        function addEntityToModel(model, entity, layer, conversionFactor) {
            var _a, _b, _c, _d;
            var id = "".concat(layer, "_").concat(Date.now(), "_").concat(Math.random());
            ...
            switch (entity.type) {
                case 'LINE':
                    if (((_a = entity.vertices) === null || _a === void 0 ? void 0 : _a[0]) && ((_b = entity.vertices) === null || _b === void 0 ? void 0 : _b[1])) {
                        model.paths["line_".concat(id)] = {
                            type: 'line',
                ...
                case 'LWPOLYLINE':
                case 'POLYLINE':
                    if (((_c = entity.vertices) === null || _c === void 0 ? void 0 : _c.length) > 1) {
                        var subModel = { paths: {} };
                        for (var i = 0; i < entity.vertices.length - 1; i++) {
                            var current = entity.vertices[i];
                            var next = entity.vertices[i + 1];
                            if (current.bulge) {
                                addBulgeArc(subModel, current, next, current.bulge, "".concat(id, "_").concat(i), conversionFactor);
                            }
                            else {
                                subModel.paths["segment_".concat(id, "_").concat(i)] = {
                                    type: 'line',
                        ...
                        if (entity.shape) {
                            ...
                            subModel.paths["close_".concat(id)] = {
                                type: 'line',
                    ...
                    break;
                case 'SPLINE':
                    if (((_d = entity.controlPoints) === null || _d === void 0 ? void 0 : _d.length) > 0) {
                        var subModel = { paths: {} };
        ...
        function addBulgeArc(model, start, end, bulge, id, conversionFactor) {
            var chord = Math.sqrt(Math.pow((end.x - start.x) * conversionFactor, 2) +
                Math.pow((end.y - start.y) * conversionFactor, 2));
            var sagitta = Math.abs(bulge) * chord / 2;
            var radius = (chord * chord / (4 * sagitta) + sagitta) / 2;
            var chordAngle = Math.atan2((end.y - start.y) * conversionFactor, (end.x - start.x) * conversionFactor);
            var bulgeAngle = Math.atan(bulge);
            var centerAngle = chordAngle + (Math.PI / 2 - bulgeAngle);
            var center = {
                x: start.x * conversionFactor + radius * Math.cos(centerAngle),
                y: start.y * conversionFactor + radius * Math.sin(centerAngle)
            };
            var startAngle = (chordAngle - Math.PI / 2 + bulgeAngle) * 180 / Math.PI;
            var endAngle = startAngle + (bulge >= 0 ? 180 : -180);
            model.paths["arc_".concat(id)] = {
                type: 'arc',
        ...
        function addSpline(model, entity, id, conversionFactor) {
            var controlPoints = entity.controlPoints.map(function (pt) { return ({
                x: pt.x * conversionFactor,
                y: pt.y * conversionFactor
            }); });
            var degree = entity.degree || 3;
            var isClosed = entity.closed;
            var knots = entity.knots || [];
            var numSegments = Math.max((controlPoints.length - 1) * 20, 100);
            var points = [];
            for (var i = 0; i <= numSegments; i++) {
                var t = i / numSegments;
                var point_1 = evaluateSpline(controlPoints, degree, knots, t);
                points.push(point_1);
            }
            for (var i = 0; i < points.length - 1; i++) {
                var current = points[i];
                var next = points[i + 1];
                model.paths["segment_".concat(id, "_").concat(i)] = {
                    type: 'line',
        ...
        function evaluateSpline(controlPoints, degree, knots, t) {
            var n = controlPoints.length - 1;
            var x = 0, y = 0;
            for (var i = 0; i <= n; i++) {
                var basis = bsplineBasis(i, degree, knots, t);
                x += controlPoints[i].x * basis;
                y += controlPoints[i].y * basis;
            }
            return { x: x, y: y };
        }
        function bsplineBasis(i, degree, knots, t) {
            if (degree === 0) {
        ...
        function chainAndMerge(targetModel, sourceModel, id) {
            var chains = MakerJs.model.findChains(sourceModel);
            if (Array.isArray(chains)) {
                chains.forEach(function (chain, index) {
                    var chainModel = MakerJs.chain.toNewModel(chain);
                    if (chainModel.paths) {
                        Object.keys(chainModel.paths).forEach(function (key) {
                            targetModel.paths["chain_".concat(id, "_").concat(index, "_").concat(key)] = chainModel.paths[key];
                        });
                    }
                });
            }
...
},{"clone":2,"dxf-parser":3,"graham_scan":4,"kdbush":5}]},{},[]);
